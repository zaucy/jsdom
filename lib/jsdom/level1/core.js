...
var core = {
  ...
  mapDOMNodes : function(parent, recursive, callback) {
    function visit(parent, result) {
      return Array.prototype.reduce.call(parent._childNodes, reducer, result);
    }
  ...
  if (src.attributes) {
    ...
    for (i=0;i<l;i++) {
      ...
      if (attr.namespaceURI) {
        ...
        copied = dest.getAttributeNodeNS(attr.namespaceURI, attr.localName);
      } else {
...
AttributeList.prototype = {
  ...
  $setNode: function (attr) {
    ...
    if (attr._ownerElement && attr._ownerElement !== this._parentNode) {
...
// Most of this file is not updated but this is updated to https://dom.spec.whatwg.org/#attr
...
core.Attr.prototype = {
  ...
  set value(v) {
    ...
    if (this._ownerElement) {
      this._ownerElement._attrModified(this._name, this._value, prev);
    }
  },
  get textContent() {
    return this.value;
...
core.DocumentType = function DocumentType(document, name) {
  core.Node.call(this, document);
  this._name = name;
  this._tagName = name;
  this._nodeName = name;

...
var fixFileUrl = require('./jsdom/utils').fixFileUrl;
...
exports.jQueryify = exports.jsdom.jQueryify = function (window, jqueryUrl, callback) {
  ...
  scriptEl.src = fixFileUrl(jqueryUrl);
...
function processHTML(config) {
  ...
  if (config.scripts.length > 0 || config.src.length > 0) {
    config.scripts.forEach(function (scriptSrc) {
      scriptSrc = fixFileUrl(scriptSrc);
...
function getConfigFromArguments(args, callback) {
  ...
  if (config.url) {
    // backslashes were originally not allowed in URLs, so this is error-handling only
    // comparable to chrome which converts all backslashes to foward slashes
